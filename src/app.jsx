...
const AREAS = {
  "القاهرة": [
    "مدينة نصر","هليوبوليس","المعادي","مصر الجديدة","الزيتون",
    "شبرا","المطرية","عين شمس","الزاوية الحمراء","النزهة",
    "الوايلي","الدراسة","باب الشعرية","الموسكي","الأزهر",
    "السيدة زينب","المنيل","دار السلام","البساتين","حلوان",
    "المعصرة","طره","قصر النيل","جاردن سيتي","الزمالك",
    "بولاق","العتبة","الأميرية","المرج","عزبة النخل"
  ],
  "الجيزة": [
    "الدقي","المهندسين","العجوزة","بولاق الدكرور","الهرم",
    "فيصل","العمرانية","الشيخ زايد","6 أكتوبر","المنيب",
    "إمبابة","كرداسة","الوراق","أوسيم","البدرشين",
    "الصف","العياط","الحوامدية","أبو النمرس"
  ],
  "القاهرة الجديدة": [
...
const GOV_COLORS={"القاهرة":"#f59e0b","الجيزة":"#10b981","القاهرة الجديدة":"#3b82f6"};
...
export default function App(){
  ...
  const submit=()=>{
    if(!form.client.trim()||!form.phone.trim()){T("ملء الاسم والهاتف مطلوب","err");return;}
    if(!form.gov||!form.area){T("من فضلك اختر المحافظة والمنطقة","err");return;}
    if(Object.values(form.items).every(q=>!q||q===0)){T("أضف صنفاً","err");return;}
    const fullName=`${form.prefix?form.prefix+" ":""}${form.client}`.trim();
    const fullAddress=`${form.gov} — ${form.area}${form.street?` — ${form.street}`:""}${form.building?` — ${form.building}`:""}${form.aptNum?` ش${form.aptNum}`:""}${form.floor?` د${form.floor}`:""}`;
    const formWithAddress={...form,client:fullName,address:fullAddress};
    ...
    mutate(d=>{
      ...
      if(ex){
        ...
        cl=cl.map(c=>c.id===ex.id?{...c,address:fullAddress,notes:form.notes,totalOrders:(c.totalOrders||0)+1,totalSpent:(c.totalSpent||0)+total}:c);
      }else{
        cid=uid("C");memberId=genMemberId(cl);
        cl=[...cl,{id:cid,memberId,name:form.client,phone:form.phone,address:fullAddress,notes:form.notes,totalOrders:1,totalSpent:total,joinedAt:Date.now()}];
      }
      const o={id:uid("ORD"),clientId:cid,memberId,...formWithAddress,address:fullAddress,total,status:"جديد",createdAt:Date.now(),deliveryDate:form.deliveryDate||null,deliveryService:null,paymentMethod:null,createdBy:ro?.label||""};
              ...
              <div key={order.id} style={{...css.card,borderColor:expOrder===order.id?ST_COLOR[order.status]:BDR,cursor:"pointer"}} onClick={()=>setExpOrder(expOrder===order.id?null:order.id)}>
                ...
                {expOrder===order.id&&(
                  <div style={{marginTop:12,borderTop:`1px solid ${BDR}`,paddingTop:12}}>
                        ...
                        <div style={{marginTop:8,paddingTop:6,borderTop:`1px solid ${BDR}`}}>
                          {role==="manager"&&(
                            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
                              <span style={{fontSize:11,color:MUT,fontWeight:600}}>خصم %</span>
                              <input
                                type="number"
                                min="0"
                                max="100"
                                ...
                                onChange={e=>{
                                  ...
                                  const orig=order._origTotal||order.total;
                                  const newTotal=Math.round(orig*(1-pct/100));
                                  mutate(d=>({...d,orders:d.orders.map(o=>o.id===order.id?{...o,total:newTotal,_origTotal:orig,discount:pct}:o)}));
                                }}
                                style={{width:60,background:"#1a2035",border:"1px solid #f59e0b",borderRadius:8,color:"#f59e0b",fontWeight:900,fontSize:14,textAlign:"center",fontFamily:"'Cairo',sans-serif",padding:"4px 8px"}}
                              />
                              {order.discount>0&&(
                              ...
                              {order.discount>0&&(
                                <button onClick={e=>{
                                  e.stopPropagation();
                                  mutate(d=>({...d,orders:d.orders.map(o=>o.id===order.id?{...o,total:o._origTotal||o.total,discount:0,_origTotal:undefined}:o)}));
                                }} style={{fontSize:10,background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontFamily:"'Cairo',sans-serif"}}>
                                  ✕ إلغاء
                                </button>
                              )}
                            </div>
                          ...
                          {order.discount>0&&order._origTotal&&(
                            <div style={{fontSize:11,color:MUT,textAlign:"left",marginTop:3}}>
                              <span style={{textDecoration:"line-through"}}>ج.م {order._origTotal}</span>
                              <span style={{color:"#10b981",marginRight:6}}>وفّرت ج.م {order._origTotal-order.total}</span>
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    {/* ✅ تأكيد الطلب — يبعت واتساب للعميل */}
                    {order.status==="جديد"&&role==="manager"&&order.phone&&(
                      <button onClick={e=>{
                        e.stopPropagation();
                        ...
                        // Move to قيد التحضير
                        advance(order.id);
                        T("✅ تم تأكيد الطلب وإرسال واتساب للعميل");
                      }} style={{...css.btn("#10b981","#fff"),marginBottom:8}}>
                        ✅ تأكيد الطلب + إشعار العميل على واتساب
                      </button>
                    )}
              ...
              <div key={c.id} style={{...css.card,borderColor:expClient===c.id?"#a78bfa":BDR,cursor:"pointer"}} onClick={()=>setExpClient(expClient===c.id?null:c.id)}>
                ...
                {expClient===c.id&&(
                  <div style={{marginTop:10,borderTop:`1px solid ${BDR}`,paddingTop:10}}>
                    ...
                    {/* Address */}
                    <div style={{fontSize:12,color:MUT,lineHeight:1.8,marginBottom:10}}>📍 {c.address}{c.notes&&<><br/>📝 {c.notes}</>}</div>

                    {/* Past orders */}
                    {(()=>{
                      const clientOrders=db.orders.filter(o=>o.clientId===c.id).sort((a,b)=>b.createdAt-a.createdAt);
                      if(!clientOrders.length)return null;
                      return(
                        <div style={{marginBottom:10}}>
                          <div style={{fontSize:11,color:MUT,marginBottom:8,fontWeight:700}}>📋 الطلبات السابقة ({clientOrders.length})</div>
                          {clientOrders.map(o=>(
                            <div key={o.id} style={{background:BG,borderRadius:10,padding:"10px 12px",marginBottom:6,border:`1px solid ${BDR}`}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                                <div>
                                  <div style={{fontSize:11,color:"#3b82f6",fontWeight:700}}>{o.id}</div>
                                  <div style={{fontSize:10,color:MUT,marginTop:1}}>
                                    {(()=>{try{const d=new Date(o.createdAt);return `${d.toLocaleDateString("ar-EG",{day:"numeric",month:"short",year:"numeric"})} — ${d.toLocaleTimeString("ar-EG",{hour:"2-digit",minute:"2-digit"})}`;}catch{return "";}})()}
                                  </div>
                                </div>
                                <div style={{textAlign:"left"}}>
                                  <div style={{fontWeight:900,fontSize:13,color:"#f59e0b"}}>ج.م {o.total}</div>
                                  <span style={{background:`${ST_COLOR[o.status]||"#374151"}22`,color:ST_COLOR[o.status]||"#94a3b8",border:`1px solid ${ST_COLOR[o.status]||"#374151"}44`,borderRadius:20,padding:"1px 7px",fontSize:9,fontWeight:700}}>{ST_ICON[o.status]} {o.status}</span>
                                </div>
                              </div>
                              {/* Order items */}
                              <div style={{borderTop:`1px solid ${BDR}`,paddingTop:6}}>
                                {Object.entries(o.items||{}).filter(([,q])=>q>0).map(([id,qty])=>{
                                  const p=products.find(x=>x.id===id);
                                  return p?(
                                    <div key={id} style={{display:"flex",justifyContent:"space-between",fontSize:11,color:MUT,padding:"2px 0"}}>
                                      <span>{p.emoji} {p.name} × {qty}</span>
                                      <span style={{color:"#f59e0b"}}>ج.م {p.price*qty}</span>
                                    </div>
                                  ):null;
                                })}
                              </div>
                              {o.address&&<div style={{fontSize:10,color:MUT,marginTop:5}}>📍 {o.address}</div>}
                            </div>
                          ))}
                        </div>
                      );
...
function OrderForm({form,setForm,setQty,products,showClient}){
  ...
  useEffect(()=>{
    ...
    if(!mapOpen&&window._kakiMap){window._kakiMap.remove();window._kakiMap=null;}
    if(mapOpen&&window._kakiMap)setTimeout(()=>window._kakiMap.invalidateSize(),200);
  },[mapOpen]);

  const initMap=()=>{
    setTimeout(()=>{
      const el=document.getElementById('kaki-map');
      if(!el||window._kakiMap)return;
      window._kakiMap=window.L.map('kaki-map').setView([30.0444,31.2357],13);
      window.L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap'}).addTo(window._kakiMap);
      const icon=window.L.divIcon({html:'<div style="font-size:32px;margin-top:-32px;margin-left:-10px">📍</div>',iconSize:[20,32],className:''});
      window._kakiMarker=window.L.marker([30.0444,31.2357],{draggable:true,icon}).addTo(window._kakiMap);
      window._kakiMap.on('click',e=>{window._kakiMarker.setLatLng(e.latlng);setLocSet(true);setForm(f=>({...f,lat:e.latlng.lat,lng:e.latlng.lng}));});
      window._kakiMarker.on('dragend',e=>{const ll=e.target.getLatLng();setLocSet(true);setForm(f=>({...f,lat:ll.lat,lng:ll.lng}));});
    },300);
  };
  ...
  const getUserLocation=()=>{
    if(navigator.geolocation){
      navigator.geolocation.getCurrentPosition(pos=>{
        ...
        setLocSet(true);setForm(f=>({...f,lat,lng}));
        if(window._kakiMap){window._kakiMap.setView([lat,lng],16);window._kakiMarker.setLatLng([lat,lng]);}
      },()=>{});
    }
  };

  const inpStyle=(field)=>({width:"100%",background:BG,border:`1px solid ${errors[field]?"#ef4444":BDR}`,borderRadius:10,padding:"10px 14px",color:"#f1f5f9",fontSize:14,fontFamily:"'Cairo',sans-serif",direction:"rtl",boxSizing:"border-box",outline:"none",marginBottom:8,boxShadow:errors[field]?"0 0 0 3px rgba(239,68,68,.15)":"none"});
  ...
  const PROP_TYPES=[{id:"apt",label:"شقة",icon:"🏢"},{id:"house",label:"فيلا",icon:"🏠"},{id:"office",label:"مكتب",icon:"🏬"}];

  return(
    <div>
      <div style={{background:CARD,borderRadius:14,padding:"14px 16px",marginBottom:10,border:`1px solid ${BDR}`}}>
        <div style={{fontWeight:800,fontSize:13,color:"#f59e0b",marginBottom:10}}>👤 بيانات العميل</div>

        <label style={lbl}>الاسم *</label>
        {/* Prefix selector */}
        <div style={{display:"flex",gap:5,marginBottom:6,flexWrap:"wrap"}}>
          {["Mr.","Mrs.","Ms.","Dr.","Eng.","Prof."].map(p=>(
            <button key={p} onClick={()=>setForm(f=>({...f,prefix:f.prefix===p?"":p}))}
              style={{background:form.prefix===p?"#f59e0b":"#0d1117",color:form.prefix===p?"#000":"#94a3b8",border:`1px solid ${form.prefix===p?"#f59e0b":"#252d3d"}`,borderRadius:20,padding:"5px 10px",fontSize:11,fontWeight:form.prefix===p?800:400,fontFamily:"'Cairo',sans-serif",cursor:"pointer"}}>
              {p}
            </button>
          ))}
        </div>
        <input style={inpStyle("client")} placeholder="الاسم" value={form.client||""} onChange={e=>{setForm(f=>({...f,client:e.target.value}));setErrors(r=>({...r,client:false}));}}/>

        <label style={lbl}>الهاتف *</label>
        <input style={inpStyle("phone")} value={form.phone||""} onChange={e=>{setForm(f=>({...f,phone:e.target.value}));setErrors(r=>({...r,phone:false}));}}/>

        {/* MAP TOGGLE */}
        <button onClick={()=>setMapOpen(o=>!o)}
          style={{width:"100%",background:locSet?"#10b98122":BG,border:`1px solid ${locSet?"#10b981":BDR}`,borderRadius:10,padding:"11px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",color:"#f1f5f9",marginBottom:8,fontFamily:"'Cairo',sans-serif"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:20}}>🗺️</span>
            <div style={{textAlign:"right"}}>
              <div style={{fontWeight:700,fontSize:13,color:locSet?"#10b981":"#f1f5f9"}}>{locSet?"✅ تم تحديد الموقع":"تحديد موقعك على الخريطة"}</div>
              <div style={{fontSize:10,color:MUT}}>اختياري</div>
            </div>
          </div>
        ...
        {mapOpen&&(
          <div style={{marginBottom:8,borderRadius:10,overflow:"hidden",border:`1px solid ${BDR}`}}>
            <div id="kaki-map" style={{width:"100%",height:220}}/>
            <div style={{padding:"8px 10px",background:"#0a0f1a",display:"flex",gap:8}}>
              <button onClick={getUserLocation}
                style={{flex:1,background:"#1e3a5f",color:"#60a5fa",border:"none",borderRadius:8,padding:"9px",fontWeight:700,fontSize:12,fontFamily:"'Cairo',sans-serif",cursor:"pointer"}}>
                📍 موقعي الحالي
              </button>
              {locSet&&(
                <button onClick={()=>setMapOpen(false)}
                  style={{flex:1,background:"#10b981",color:"#fff",border:"none",borderRadius:8,padding:"9px",fontWeight:800,fontSize:12,fontFamily:"'Cairo',sans-serif",cursor:"pointer"}}>
                  📍 هذا موقعي ✅
                </button>
              )}
            </div>
            <div style={{fontSize:10,color:MUT,textAlign:"center",padding:"4px",background:"#0a0f1a"}}>اضغط على الخريطة أو اسحب الـ Pin</div>
          </div>
        )}
        ...
        <div style={{display:"flex",gap:6,marginBottom:8}}>
          {Object.keys(AREAS).map(g=>(
            <button key={g} onClick={()=>setForm(f=>f.gov===g?{...f,gov:"",area:""}:{...f,gov:g,area:""})}
              style={{flex:1,background:form.gov===g?(GOV_COLORS[g]||"#f59e0b"):BG,color:form.gov===g?"#000":MUT,border:`1px solid ${errors.gov?"#ef4444":form.gov===g?(GOV_COLORS[g]||"#f59e0b"):BDR}`,borderRadius:9,padding:"9px 4px",fontWeight:700,fontSize:10,fontFamily:"'Cairo',sans-serif",cursor:"pointer"}}>
              {g} {form.gov===g?"▲":"▼"}
            </button>
          ))}
        </div>
        ...
        {form.gov&&!form.area&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,maxHeight:180,overflowY:"auto",marginBottom:8}}>
            {AREAS[form.gov].map(a=>(
              <button key={a} onClick={()=>{setForm(f=>({...f,area:a}));setErrors(r=>({...r,area:false}));}}
                style={{background:BG,color:"#f1f5f9",border:`1px solid ${errors.area?"#ef4444":BDR}`,borderRadius:8,padding:"8px 6px",fontSize:11,fontFamily:"'Cairo',sans-serif",cursor:"pointer",textAlign:"center"}}>
                {a}
              </button>
            ))}
          </div>
        ...
        {form.gov&&form.area&&(
          <button onClick={()=>setForm(f=>({...f,area:""}))}
            style={{width:"100%",background:GOV_COLORS[form.gov]||"#f59e0b",color:"#000",border:"none",borderRadius:9,padding:"10px 14px",fontWeight:800,fontSize:13,fontFamily:"'Cairo',sans-serif",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <span>📍 {form.gov} — {form.area}</span>
        ...
        {form.area&&(
          <>
            <div style={{display:"flex",gap:8,marginBottom:10}}>
              {PROP_TYPES.map(t=>(
                <button key={t.id} onClick={()=>setForm(f=>({...f,propType:t.id}))}
                  style={{flex:1,background:(form.propType||"apt")===t.id?"#fff":BG,color:(form.propType||"apt")===t.id?"#000":MUT,border:`1px solid ${(form.propType||"apt")===t.id?"#fff":BDR}`,borderRadius:20,padding:"9px 6px",fontWeight:(form.propType||"apt")===t.id?800:400,fontSize:12,fontFamily:"'Cairo',sans-serif",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
                  {t.icon} {t.label}
                </button>
              ))}
            </div>

            <label style={lbl}>اسم العمارة *</label>
            <input style={inpStyle("building")} placeholder="مثال: عمارة النيل" value={form.building||""} onChange={e=>{setForm(f=>({...f,building:e.target.value}));setErrors(r=>({...r,building:false}));}}/>

            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <div style={{flex:1}}>
                <label style={lbl}>رقم الشقة *</label>
                <input style={{...inpStyle("aptNum"),marginBottom:0}} placeholder="٣" value={form.aptNum||""} onChange={e=>{setForm(f=>({...f,aptNum:e.target.value}));setErrors(r=>({...r,aptNum:false}));}}/>
              </div>
              <div style={{flex:1}}>
                <label style={lbl}>الدور (اختياري)</label>
                <input style={{...inpStyle(),marginBottom:0}} placeholder="٢" value={form.floor||""} onChange={e=>setForm(f=>({...f,floor:e.target.value}))}/>
              </div>
            </div>
            <div style={{height:8}}/>

            <label style={lbl}>الشارع *</label>
            <input style={inpStyle("street")} placeholder="مثال: شارع الجمهورية" value={form.street||""} onChange={e=>{setForm(f=>({...f,street:e.target.value}));setErrors(r=>({...r,street:false}));}}/>

            <label style={lbl}>ملاحظات إضافية (اختياري)</label>
            <input style={inpStyle()} placeholder="أي تفاصيل..." value={form.details||""} onChange={e=>setForm(f=>({...f,details:e.target.value}))}/>
          </>
        )}

        <label style={lbl}>ملاحظات الطلب</label>
        <input style={inpStyle()} value={form.notes||""} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/>
      </div>
